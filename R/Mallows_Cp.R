...
Mallows_Cp <- function(formula, data = list(), model = TRUE, x = FALSE,
  ...
  if (var.full<0) {
    if (verbose) cat("Mallows Cp: the variance of the full model can not be negative, using default value \n")
    var.full <- 0
  }
  ...
  class(result) <- "Mallows Cp"

  return(result)

}

summary.Mallows.Cp <- function (object, num.max=20, verbose=FALSE, ...) {

  if (is.null(object$terms)) {
    stop("invalid \'Mallows Cp\' object")
  }
...
print.Mallows.Cp <- function (x, digits = max(3, getOption("digits") - 3),  num.max=max(1, nrow(x$cp)), ...) {
